...
class RetroTerminal {
  ...
  init() {
    ...
    this.root = document.getElementById('crt-shell');
  ...
  async bootSequence() {
    ...
    await writeBootLog(`<span class="dim">[ 0.000000]</span> ${(window.SITE_TITLE || 'Terminal').toUpperCase()} BIOS v1.82.04 <span class="dim">(build 2024.11.21)</span>`, 'bright', 3);
    ...
    await this.writeLineAsync(`\n<span class="bright">guest@${window.SITE_TITLE_SLUG || 'terminal'}</span> connected successfully.`, 'system', 8);
  ...
  handleAutocomplete() {
    ...
    if (suggestions.length === 1) {
      ...
      this.writeLine(`guest@${window.SITE_TITLE_SLUG || 'terminal'}:~$ ${this.input.value}`, 'dim');
  ...
  async processCommand(rawInput) {
    ...
    this.writeLine(`guest@${window.SITE_TITLE_SLUG || 'terminal'}:~$ ${rawInput}`);
